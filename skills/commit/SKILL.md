...
- 변경사항이 없는 레포는 **커밋만** 건너뛴다(빈 커밋을 만들지 않음). develop 동기화(pull)는 변경 유무와 무관하게 모든 대상 레포에서 수행한다.
- 레포 경로: `C:\workspace\bgt\bgt-fe`, `C:\workspace\bgt\bgt-be`.

## Workflow

먼저 **모든 대상 레포에서 develop 을 동기화**한 뒤(변경 유무와 무관), 변경이 있는 레포만 커밋한다.

### 1. develop 동기화 (대상 레포 전체 · 변경 유무와 무관 · 생략 불가)

각 대상 레포에서 `git -C <repo> pull origin develop` 을 실행해 현재 브랜치에 develop 최신 변경을 병합한다.

- **커밋할 변경이 없는 레포도 반드시 pull 한다.** develop 동기화를 "커밋 대상 여부"로 건너뛰지 않는다(이 게이팅이 백엔드가 develop 소스를 못 받던 원인).
- 충돌이 나면 변경사항이 남지 않도록 소스를 적절히 수정해 충돌을 해소한 뒤 진행한다. 단, 충돌·수정 규모가 크면 작업을 중단하고 사용자에게 보고한다.

### 2. 커밋 (변경이 있는 레포만)

각 대상 레포에서 순서대로 처리한다:

1. `git -C <repo> status -s` 로 변경을 확인한다. 변경이 없으면 그 레포의 **커밋만** 건너뛴다(develop 동기화는 1단계에서 이미 완료).
