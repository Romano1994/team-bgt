...
### 5. 커밋
1. `git add skills/<name> [skills/<name2> ...]` — 발행 대상만. `git add -A` 로 뭉뚱그리지 않는다.
2. 한국어 컨벤션 메세지: `feat: <스킬명> 스킬 추가` (여러 개면 나열, 기존 스킬 수정이면 `fix:`). 끝에:
   ```
...
- 플러그인 `version` 업은 이 스킬에서 다루지 않는다(PR 머지 시 별도). 필요하면 사용자가 명시.
