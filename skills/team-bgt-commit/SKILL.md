...
description: Use when 개인 하네스 폴더(~/.claude/{skills,agents,commands,hooks})에서 개발·테스트한 아티팩트(스킬·에이전트·커맨드·훅)를 team-bgt 플러그인 레포에 발행(퍼블리시)할 때. 트리거 "/team-bgt-commit <이름>", "스킬/훅/에이전트/커맨드 발행/퍼블리시", "내 하네스 team-bgt에 올려/커밋", "하네스 PR". 이름 인자를 주면 그 아티팩트만 개인 하네스에서 발행하고, 인자 없이 부르면 team-bgt 레포에 이미 있는 아티팩트 파일들의 변경사항 전체를 커밋한다. bgt-fe/bgt-be 소스 커밋(=commit 스킬)이 아니고, 플러그인 레포 전체를 main 에 직접 커밋하는 것도 아니다 — 아티팩트 단위로 브랜치를 파고 PR 을 만든다.
---
...
- **인자 있으면 그 이름만, 없으면 레포 변경 전체.** 이름을 주면 개인 하네스에서 그 아티팩트를 발행한다(발행 모드). 인자 없이 호출되면 묻지 말고 team-bgt 레포에 **이미 존재하는** 아티팩트 파일들의 변경사항(수정·신규)을 전부 커밋한다(레포 커밋 모드 — 개인 폴더 복사·삭제 생략).
...
### 0. 인자 읽기 → 모드 결정
호출 인자에서 이름을 읽는다. 공백/쉼표로 **여러 개**일 수 있다.
- **이름 있음 → 발행 모드**: 개인 하네스 → 플러그인으로 복사·발행. 각 이름을 위 4개 개인 경로에 프로브해 타입을 판별한다. `타입:이름` 으로 명시됐으면 그 타입만 본다. 여러 타입에 걸리면 AskUserQuestion. 1·4·7단계 포함 전체 흐름.
- **이름 없음 → 레포 커밋 모드**: 묻지 않는다. 개인 폴더 복사·삭제(1·4·7단계)를 건너뛰고, team-bgt 레포에 **이미 있는** 아티팩트의 변경사항 전체를 커밋한다 → 아래 **0b** 로.

### 0b. 레포 커밋 모드 (인자 없이 호출됐을 때)
개인 폴더는 손대지 않고, team-bgt 레포 자체의 워킹트리 변경만 커밋한다.
1. **team-bgt 위치 찾기** — 2단계와 동일.
2. `git -C <repo> status --porcelain` 로 `skills/`·`agents/`·`commands/`·`hooks/`·`.claude-plugin/plugin.json` 아래 변경(수정·신규)을 모은다. 변경이 하나도 없으면 "커밋할 변경사항이 없습니다" 로 중단.
3. **브랜치** — 3단계와 동일하되 이름은 변경된 아티팩트에서 유도: 1개면 그 이름, 여러 개면 대표(첫 번째, 사용자가 브랜치명을 줬으면 그것). main 최신 확보 후 `git switch -c <branch>` (워킹트리 변경은 브랜치에 따라온다).
4. **버전업 + 커밋** — 5단계와 동일. 변경 성격으로 등급 판단(수정만 = `fix:`/PATCH, 새 아티팩트 포함 = `feat:`/MINOR, 계약·트리거 깨짐 = MAJOR), `plugin.json` version 올림. `git add` 는 변경된 아티팩트 경로 + `plugin.json` 만(`-A` 금지).
5. **push + PR** — 6단계와 동일.
- 7단계(원본 삭제)는 **하지 않는다** — 옮긴 게 아니라 레포 안 변경을 커밋한 것.

### 1. 검증 (소유권 + 존재) — 발행 모드, 이름마다
...
- **인자 없는데 이름을 묻기** → 인자 없음은 '레포 변경 전체 커밋' 모드다. 묻지 말고 레포 커밋 모드(0b)로 진행.
